...
func serveMarkdown(input string, filename string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		resp, err := http.Post("https://api.github.com/markdown/raw", "text/plain", file)
		if err != nil {
			log.Fatal(err)
		}
		...
		log.Println("Building preview")
		p := page{fmt.Sprintf("mdpreview - %s", filename), string(body)}
		renderTemplate(w, p)
	}
}

func isValidMarkdown(ext string) bool {
	markdownExtensions := []string{".markdown", ".mdown", ".mkdn", ".md", ".mkd", ".mdwn", ".mdtxt", ".mdtext"}

	for _, a := range markdownExtensions {
		if a == ext {
			return true
		}
	}
...
func main() {
	...
	flag.StringVar(&input, "i", "README.md", "Markdown file to render")
	flag.Usage = func() {
		fmt.Printf("Usage\n")
		fmt.Printf("  $ mdpreview [options]\n")
		fmt.Printf("\n")
		fmt.Printf("Options\n")
		flag.PrintDefaults()
		fmt.Printf("\n")
		fmt.Printf("Examples\n")
		fmt.Printf("  $ mdpreview\n")
		fmt.Printf("  $ mdpreview -f some/file.md\n")
	}

	flag.Parse()

	if !isValidMarkdown(filepath.Ext(input)) {
		log.Fatal(fmt.Sprintf("File %s is not a valid markdown", input))
	}

	_, err := os.Stat(input)
	if os.IsNotExist(err) {
		log.Fatal(fmt.Sprintf("File %s does not exist.", input))
	}

	filename := filepath.Base(input)

	http.HandleFunc("/", serveMarkdown(input, filename))
	...
	log.Printf("Serving %s preview at %s\n", filename, url)
...
func renderTemplate(wr io.Writer, p page) {
	t, err := template.New("md").Parse(pageTemplate)
	if err != nil {
		log.Fatal(err)
	}

	err = t.ExecuteTemplate(wr, "T", p)
	if err != nil {
		log.Fatal(err)
	}
}
